...
@implementation BKProgressBarView
...
- (void)initializeUserInterface
{
    if (self.type == progressBar) {//条形进度条
        ...
        //初始化外环layer的path
        UIBezierPath *loopPath = [UIBezierPath bezierPathWithArcCenter:center radius:MIN(self.bounds.size.width - 2, self.bounds.size.height - 2) / 2.0 startAngle:-M_PI_2 endAngle:M_PI * 3.0 / 2.0 clockwise:YES];
        ...
        if (self.type == loopProgressBar) {//环形进度条
            ...
            roundLayer.path = loopPath.CGPath;
            roundLayer.strokeColor = [UIColor groupTableViewBackgroundColor].CGColor;
        ...
        if (self.type == circularProgressBar) {
            [path addArcWithCenter:center radius:MIN(self.bounds.size.width - 2, self.bounds.size.height - 2) / 2.0 startAngle:-M_PI_2 endAngle:-M_PI_2 clockwise:YES];
            [self.layer addSublayer:self.progressBarLayer];
...
- (void)setBarType:(ProgressBarType)barType {
    ...
    if (barType == progressBar) {//条形进度条
        ...
        if (barType == loopProgressBar) {//环形进度条
            [self.layer addSublayer:self.outerRingLayer];
        }
        
        //初始化圆形以及内胆形进度条的path
        UIBezierPath *path = [UIBezierPath bezierPath];
        if (barType == circularProgressBar) {
            ...
            [self.layer addSublayer:self.progressBarLayer];
        }else if (barType == tankFormProgressBar) {
            [path addArcWithCenter:center radius:MIN(self.bounds.size.width, self.bounds.size.height) / 2.0 startAngle:-M_PI_2 endAngle:-M_PI_2 clockwise:YES];
            self.outerRingLayer.strokeEnd = 1;
            self.outerRingLayer.backgroundColor = [UIColor clearColor].CGColor;
            [self.progressBarLayer addSublayer:self.outerRingLayer];
